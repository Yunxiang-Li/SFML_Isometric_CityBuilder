...
MainMenuGameState::MainMenuGameState(std::unique_ptr<Game> game_ptr)
{
	...
	// Set game view's center(origin of all transformations) to be the center position of the window.
	...
	while (m_game_ptr->m_game_window.pollEvent(event))
	{
		...
		switch(event.type)
		{
			...
			case sf::Event::Resized:
			{
				...
				// Set background position to window position (0, 0) related world position inside default(main menu) view.
