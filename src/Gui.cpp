...
int Gui::get_gui_entry_idx(const sf::Vector2f& mouse_pos) const
{
	...
	for (size_t i = 0; i < m_gui_entry_vec.size(); ++i)
	{
		...
		pos -= m_gui_entry_vec[i].m_entry_shape.getPosition();
		...
		pos += m_gui_entry_vec[i].m_entry_shape.getOrigin();

		// Ignore all positions that are not within the relative GuiEntry object's shape.
		if (pos.x < 0 || pos.x > (m_gui_entry_vec[i].m_entry_shape.getScale().x * m_entry_shape_dimension.x))
			continue;
		if (pos.y < 0 || pos.y > (m_gui_entry_vec[i].m_entry_shape.getScale().y * m_entry_shape_dimension.y))
			continue;
	}
